...
#' @examples
#' # 1D example:
cv_unique_fof_par_v2 <- function(X,
                                 Y,
                                 center = TRUE,
                                 argvals_X = NULL,
                                 argvals_Y = NULL,
  ...
  for (ncomp_i in 1:ncomp) {
    ...
    MSE_lambda_fold <- foreach::foreach (i = 1:num_folds,
                       ...
                       .combine = 'c' ) %dopar%
      {
        ...
        basis_repr_Y_fold_test <- fda::Data2fd(argvals = argvals_Y,
                                               y = t(Y_fold_test),
                                               basisobj = basisobj_Y  )

        Y_fold_test <- Matrix::t(fda::eval.fd(evalarg = argvals_Y,
                                              fdobj = basis_repr_Y_fold_test,
                                              Lfdobj = 0,
                                              returnMatrix = TRUE))
